...
VideoCapture::Frame *WhoDecodeNode::process(VideoCapture::Frame *fb)
{
    ...
    return new VideoCapture::Frame(
        img.data, img.bytes(), img.width, img.height, dl_pix_type2v4l2_fmt(img.pix_type), timestamp);
}
...
VideoCapture::Frame *WhoPPAResizeNode::process(VideoCapture::Frame *fb)
{
    ...
    return new VideoCapture::Frame(dst_img.data,
                                   dst_img.bytes(),
                                   dst_img.width,
                                   dst_img.height,
                                   dl_pix_type2v4l2_fmt(dst_img.pix_type),
                                   timestamp);
}
