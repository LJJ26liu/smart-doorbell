...
esp_err_t V4L2Device::set_format(enum v4l2_buf_type type,
                                 uint32_t width,
                                 uint32_t height,
...
esp_err_t V4L2Device::request_buffers(enum v4l2_buf_type type,
...
esp_err_t V4L2Device::query_buffer(enum v4l2_buf_type type,
                                   uint32_t index,
                                   ...
                                   struct v4l2_buffer *buf)
{
    ...
    if (::ioctl(fd_, VIDIOC_QUERYBUF, buf) != 0) {
        ESP_LOGE(TAG, "Failed to query buffer %" PRIu32 ": %s", index, strerror(errno));
        return ESP_FAIL;
    }
...
esp_err_t V4L2Device::dequeue_buffer(enum v4l2_buf_type type,
                                     enum v4l2_memory mem,
                                     struct v4l2_buffer *buf,
                                     uint32_t timeout_ms)
{
...
esp_err_t V4L2Device::enum_frame_sizes(enum v4l2_buf_type type,
                                       uint32_t index,
                                       uint32_t pixel_format,
...
esp_err_t V4L2Device::enum_frame_intervals(enum v4l2_buf_type type,
                                           uint32_t index,
                                           uint32_t pixel_format,
                                           uint32_t width,
                                           uint32_t height,
