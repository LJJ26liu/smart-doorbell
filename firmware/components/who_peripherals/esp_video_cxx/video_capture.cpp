...
esp_err_t VideoCapture::Config::validate() const
{
    ...
    if (buffer_mem_type != V4L2_MEMORY_MMAP && buffer_mem_type != V4L2_MEMORY_USERPTR) {
        ESP_LOGE(TAG, "Invalid buffer memory type");
        return ESP_ERR_INVALID_ARG;
    }

    auto is_uvc_device = [](const char *name) -> bool {
        // UVC devices are /dev/video40 to /dev/video49
        return (strncmp(name, "/dev/video4", 11) == 0 && name[11] >= '0' && name[11] <= '9');
    };
...
esp_err_t VideoCapture::init(const Config &cfg)
{
    ...
    if (is_uvc) {
        ...
        if (auto ret = device_.set_format(V4L2_BUF_TYPE_VIDEO_CAPTURE, uvc_cfg.width, uvc_cfg.height, cfg.pixel_format);
            ret != ESP_OK) {
            print_frame_params();
            ESP_ERROR_CHECK(device_.close());
            return ret;
        }
        if (auto ret = device_.set_fps_by_denominator(V4L2_BUF_TYPE_VIDEO_CAPTURE, uvc_cfg.fps, 10000000UL);
            ret != ESP_OK) {
            print_frame_params();
            ESP_ERROR_CHECK(device_.close());
            return ret;
        ...
        if (cfg.pixel_format != format.fmt.pix.pixelformat) {
            if (auto ret = device_.set_format(
                    V4L2_BUF_TYPE_VIDEO_CAPTURE, format.fmt.pix.width, format.fmt.pix.height, cfg.pixel_format);
                ret != ESP_OK) {
                print_frame_params();
                ESP_ERROR_CHECK(device_.close());
                return ret;
    ...
    // Request buffers
    if (auto ret = device_.request_buffers(V4L2_BUF_TYPE_VIDEO_CAPTURE, cfg.buffer_mem_type, cfg.buffer_count);
        ret != ESP_OK) {
    ...
    for (uint32_t i = 0; i < cfg.buffer_count; i++) {
        ...
        if (auto ret = device_.query_buffer(V4L2_BUF_TYPE_VIDEO_CAPTURE, i, cfg.buffer_mem_type, &buf); ret != ESP_OK) {
            ESP_LOGD(TAG, "Failed to query buffer %" PRIu32, i);
            cleanup(cfg);
            return ret;
        }
        ...
        if (cfg.buffer_mem_type == V4L2_MEMORY_MMAP) {
            if (auto ret = device_.map_buffer(buf.m.offset, buf.length, &buf_addr); ret != ESP_OK) {
                ESP_LOGD(TAG, "Failed to map buffer %" PRIu32, i);
                cleanup(cfg);
                return ret;
            }
            ...
            if (!buf_addr) {
                ESP_LOGD(TAG, "Failed to alloc buffer %" PRIu32, i);
                cleanup(cfg);
        ...
        if (auto ret = device_.queue_buffer(V4L2_BUF_TYPE_VIDEO_CAPTURE, buf); ret != ESP_OK) {
            ESP_LOGD(TAG, "Failed to queue buffer %" PRIu32, i);
            cleanup(cfg);
            return ret;
        }
    }
...
esp_err_t VideoCapture::get_frame(Frame **frame, uint32_t timeout_ms)
{
    ...
    if (auto ret = device_.dequeue_buffer(V4L2_BUF_TYPE_VIDEO_CAPTURE, cfg_.buffer_mem_type, &buf, timeout_ms);
        ret != ESP_OK) {
        *frame = nullptr;
        return ret;
    }
...
std::vector<VideoCapture::FrameParams> VideoCapture::enum_frame_params()
{
    ...
    for (uint32_t fmt_index = 0;; fmt_index++) {
        ...
        for (;; fmt_frmsize_index++) {
            ...
            if (device_.enum_frame_sizes(
                    V4L2_BUF_TYPE_VIDEO_CAPTURE, fmt_frmsize_index, fmtdesc.pixelformat, &frmsize) != ESP_OK) {
                break;
            }
            uint32_t fmt_frmival_index = 0;
            for (;; fmt_frmival_index++) {
                struct v4l2_frmivalenum frmival;
                if (device_.enum_frame_intervals(V4L2_BUF_TYPE_VIDEO_CAPTURE,
                                                 fmt_frmival_index,
                                                 fmtdesc.pixelformat,
                ...
                params.emplace_back(fmtdesc.pixelformat,
        ...
        if (fmt_frmsize_index == 0) {
            ...
            device_.get_fps(V4L2_BUF_TYPE_VIDEO_CAPTURE, &fps);
            params.emplace_back(
                fmtdesc.pixelformat, (char *)fmtdesc.description, v4l2_fmt.fmt.pix.width, v4l2_fmt.fmt.pix.height, fps);
        }
    }
    return params;
}
