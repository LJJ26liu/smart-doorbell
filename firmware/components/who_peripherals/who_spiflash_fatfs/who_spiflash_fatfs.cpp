...
esp_err_t spiflash_fatfs_mount()
{
    ...
    ESP_RETURN_ON_ERROR(esp_vfs_fat_spiflash_mount_rw_wl(
                        ...
                        TAG,
                        "Failed to mount spiflash fatfs.");
    return ESP_OK;
}

esp_err_t spiflash_fatfs_unmount()
{
    ESP_RETURN_ON_ERROR(esp_vfs_fat_spiflash_unmount_rw_wl(CONFIG_SPIFLASH_MOUNT_POINT, wl_handle),
                        TAG,
                        "Failed to unmount spiflash fatfs.");
    return ESP_OK;
}
